...
namespace osuCrypto
{
    ...
    void EACode_encode_basic_test(const oc::CLP& cmd)
    {
        ...
        auto k = cmd.getOr("k", 1<<25);
        auto R = cmd.getOr("R", 5.0);
        auto n = cmd.getOr("n", k * R);
        auto bw = cmd.getOr("bw", 9);
...
void EACode_encode_basic_test(const oc::CLP& cmd)
    {
        ...
        auto k = cmd.getOr("k", 1<<20);
        auto R = cmd.getOr("R", 5.0);
        auto n = cmd.getOr("n", k * R);
        auto bw = cmd.getOr("bw", 40);
        ...
        //auto B = code.getB();
        //auto G = B * A;
        std::vector<u64> m0(k), m1(k), c(n), c0(n), c1(n), a1(n);
        std::vector<u8> c2(n), m2(k);
        ...
        auto a0 = c0;
        code.accumulate<u64, CoeffCtxInteger>(a0, {});
        CoeffCtxInteger ctx;
        auto sum = c0[0];
        ...
        code.dualEncode<u64, CoeffCtxInteger>(c0, m1, {});
...
void EACode_encode_basic_test(const oc::CLP& cmd)
    {
        ...
        auto a0 = c0;
        code.accumulate<u64, CoeffCtxIntegerPrime>(a0, {});
        ...
        code.dualEncode<u64, CoeffCtxIntegerPrime>(c0, m1, {});
