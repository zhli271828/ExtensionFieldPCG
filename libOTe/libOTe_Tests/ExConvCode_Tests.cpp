...
namespace osuCrypto
{
    ...
    {
        auto K = cmd.getManyOr<u64>("k", { 1<<20});
        auto R = cmd.getManyOr<double>("R", { 2.0});
        auto Bw = cmd.getManyOr<u64>("bw", { 7});
        auto Aw = cmd.getManyOr<u64>("aw", { 16 });

        //bool v = cmd.isSet("v");
        for (auto k : K) for (auto r : R) for (auto bw : Bw) for (auto aw : Aw) for (auto sys : {  false })
        {
            auto n = k * r;
